<template>
  ...
  <p class="text-center text-h4" v-if="sum >= 0">{{ t('pay', { sum }) }}</p>
  <p class="text-center text-h4" v-else>{{ t('get', { sum: sum * -1 }) }}</p>
  <q-btn
    :label="t('updateMarket')"
    color="dark"
    @click="if (newMarketState !== null) currentMarketState = newMarketState - 1;"
  />
</template>
...
import MarketBuyItem from '../components/MarketBuyItem.vue';
...
const items = computed(() => {
  ...
  return marketInfos[props.marketType].slice(newMarketState.value - 1, currentMarketState.value);
});
